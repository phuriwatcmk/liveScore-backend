...
news.get("/", zValidator("query", newsQuerySchema), (c) => {
    try {
        ...
        // Filter by category
        if (category) {
            filteredNews = filteredNews.filter((article) => article.category.toLowerCase().includes(category.toLowerCase()));
        }
        // Filter by tags
        if (tags) {
            const tagList = tags.split(",").map((tag) => tag.trim().toLowerCase());
            filteredNews = filteredNews.filter((article) => article.tags.some((tag) => tagList.some((searchTag) => tag.toLowerCase().includes(searchTag))));
        }
        ...
        return c.json(createSuccessResponse(data, "News retrieved successfully", pagination));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve news"), 500);
    }
});
...
news.get("/categories", (c) => {
    try {
        ...
        return c.json(createSuccessResponse(categories, "News categories retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve news categories"), 500);
    }
});
...
news.get("/tags", (c) => {
    try {
        ...
        return c.json(createSuccessResponse(uniqueTags, "News tags retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve news tags"), 500);
    }
});
...
news.get("/trending", zValidator("query", newsQuerySchema), (c) => {
    try {
        ...
        return c.json(createSuccessResponse(data, "Trending news retrieved successfully", pagination));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve trending news"), 500);
    }
});
...
news.get("/:newsId", zValidator("param", newsIdSchema), (c) => {
    try {
        ...
        if (!article) {
            return c.json(createErrorResponse("NOT_FOUND", `News article with ID ${newsId} not found`), 404);
        }
        ...
        return c.json(createSuccessResponse(articleWithIncrementedViews, "News article retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve news article"), 500);
    }
});
...
news.get("/:newsId/related", zValidator("param", newsIdSchema), (c) => {
    try {
        ...
        const relatedNews = mockNews
            ...
            .filter((n) => n.category === article.category || // Same category
        ...
        return c.json(createSuccessResponse(relatedNews, "Related news articles retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve related news articles"), 500);
    }
});
export default news;
