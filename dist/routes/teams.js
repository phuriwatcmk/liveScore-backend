...
teams.get("/:teamId", (c) => {
    try {
        ...
        if (!teamData) {
            return c.json(createErrorResponse("NOT_FOUND", `Team with ID ${teamId} not found`), 404);
        }
        return c.json(createSuccessResponse(teamData, "Team overview retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve team overview"), 500);
    }
});
...
teams.get("/:teamId/matches", (c) => {
    try {
        ...
        if (teamMatches.length === 0) {
            return c.json(createErrorResponse("NOT_FOUND", `No matches found for team ${teamId}`), 404);
        }
        const { data, pagination } = paginate(teamMatches, page, limit);
        return c.json(createSuccessResponse(data, "Team matches retrieved successfully", pagination));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve team matches"), 500);
    }
});
...
teams.get("/:teamId/standings", (c) => {
    try {
        ...
        const leagueStandings = mockStandingsByLeague.find((standing) => standing.league.id === teamLeagueId);
        const standings = leagueStandings?.standings;
        if (!standings) {
            return c.json(createErrorResponse("NOT_FOUND", `Standings not found for team's league`), 404);
        }
        ...
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve team standings"), 500);
    }
});
...
teams.get("/:teamId/statistics", (c) => {
    try {
        ...
        return c.json(createSuccessResponse(statistics, "Team statistics retrieved successfully"));
    }
    catch (error) {
        return c.json(createErrorResponse("INTERNAL_ERROR", "Failed to retrieve team statistics"), 500);
    }
});
