  ...
  describe("GET /api/v1/h2h/:team1Id/:team2Id", () => {
    ...
    it("should work with teams in reverse order", async () => {
      ...
      if (res1.status === 200 && res2.status === 200) {
        ...
        expect(data1.data.statistics.totalMatches).toBe(
          data2.data.statistics.totalMatches
        );
      }
    });
    ...
    it("should calculate statistics correctly", async () => {
      ...
      if (res.status === 200) {
        ...
        expect(stats.team1.wins + stats.team2.wins + stats.draws).toBe(
          stats.totalMatches
        );
        expect(
          stats.team1.winPercentage +
    ...
    it("should return recent matches between teams", async () => {
      ...
      if (res.status === 200) {
        ...
        if (data.data.length > 1) {
          ...
          const dates = data.data.map((match: any) =>
