...
describe("News API", () => {
  describe("GET /api/v1/news", () => {
        ...
        expect(
          data.data.every((article: any) =>
            article.tags.some((tag: string) =>
              ["haaland", "premier league"].some((searchTag) =>
    ...
    it("should search news by title and summary", async () => {
      ...
      if (data.data.length > 0) {
        expect(
          data.data.every(
            (article: any) =>
              article.title.toLowerCase().includes("haaland") ||
              article.summary.toLowerCase().includes("haaland")
          )
        ).toBe(true);
      }
    });
    ...
    it("should return news sorted by date (most recent first)", async () => {
      ...
      if (data.data.length > 1) {
        const dates = data.data.map((article: any) =>
  ...
  describe("GET /api/v1/news/trending", () => {
    it("should return trending news sorted by views", async () => {
      ...
      if (data.data.length > 1) {
        const views = data.data.map((article: any) => article.views);
  ...
  describe("GET /api/v1/news/:newsId/related", () => {
    it("should return related news articles", async () => {
      ...
      expect(data.data.every((article: any) => article.id !== 1)).toBe(true);
    });
