...
leagues.get("/", zValidator("query", paginationSchema), (c) => {
  try {
    ...
    return c.json(
      createErrorResponse("INTERNAL_ERROR", "Failed to retrieve leagues"),
      500
    );
  }
});
...
leagues.get("/:leagueId", zValidator("param", leagueParamsSchema), (c) => {
  try {
    ...
    return c.json(
      createSuccessResponse(league, "League retrieved successfully")
    );
  } catch (error) {
    return c.json(
      createErrorResponse("INTERNAL_ERROR", "Failed to retrieve league"),
      500
    );
  }
});
    ...
    try {
      ...
      if (!standings) {
        return c.json(
          createErrorResponse(
            "NOT_FOUND",
            `Standings for league ${leagueId} not found`
          ),
          404
        );
      }
      ...
      return c.json(
        createSuccessResponse(standings, "Standings retrieved successfully")
      );
    } catch (error) {
      return c.json(
        createErrorResponse("INTERNAL_ERROR", "Failed to retrieve standings"),
        500
      );
    }
  }
