...
h2h.get(
  "/:team1Id/:team2Id",
  zValidator("param", h2hParamsSchema),
    ...
    try {
      ...
      if (h2hRecords.length === 0) {
        return c.json(
          createErrorResponse(
            "NOT_FOUND",
            `No head-to-head records found between teams ${team1Id} and ${team2Id}`
          ),
          404
        );
      }
      ...
      const totalMatches = h2hRecords.reduce(
        (sum, record) => sum + record.fixtures.length,
        0
      );
      ...
      return c.json(
        createSuccessResponse(
          response,
          "Head-to-head records retrieved successfully",
          pagination
        )
      );
    } catch (error) {
      return c.json(
        createErrorResponse(
          "INTERNAL_ERROR",
          "Failed to retrieve head-to-head records"
        ),
        500
      );
    }
    ...
    try {
      ...
      if (h2hRecords.length === 0) {
        return c.json(
          createErrorResponse(
            "NOT_FOUND",
            `No recent matches found between teams ${team1Id} and ${team2Id}`
          ),
          404
        );
      }
      ...
      return c.json(
        createSuccessResponse(
          allFixtures,
          "Recent head-to-head matches retrieved successfully"
        )
      );
    } catch (error) {
      return c.json(
        createErrorResponse(
          "INTERNAL_ERROR",
          "Failed to retrieve recent head-to-head matches"
        ),
        500
      );
    }
